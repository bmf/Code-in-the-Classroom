...
var bounceChar = function() {
  $('.currentLetter').css({
    'left': ($('#originalText').position().left - 10) + 'px',
    'top': ($('#originalText').position().top - 20) + 'px'
  }).animate({
    top: '+=' + 5
  }, 200, 'easeOutBounce').animate({
    top: '-=' + 5
  }, 200, 'easeOutBounce');
...
$(function() {
  ...
  $(window).keydown(function(e) {
    ...
    if (String.fromCharCode(e.keyCode).toUpperCase() == characterArray[0]) {
      ...
      $('.staticLetter').css({
        'left': $('#originalText').position().left + 'px',
        'top': $('#originalText').position().top + 'px'
      }).animate({
        top: '+=' + 300
      }, 500, 'easeOutBounce').fadeOut('slow');
